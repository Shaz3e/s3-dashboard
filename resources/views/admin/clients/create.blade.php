...
@section('content')
    <x-page-title title="{{ __('client.title.create') }}" :breadcrumbs="[
        ['url' => '/', 'label' => __('app.breadcrumb.dashboard')],
        ['url' => route('admin.clients.index'), 'label' => __('client.breadcrumb.index')],
        ['label' => __('client.breadcrumb.create')],
    ]" />

    <form action="{{ route('admin.clients.store') }}" method="POST" class="needs-validation" novalidate>
        @csrf
        <div class="row">
            <div class="col-md-12">
                <div class="card">
